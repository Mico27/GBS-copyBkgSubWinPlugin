export const id = "EVENT_COPY_BKG_SUBMAP_TO_BKG_BASE_LEGACY";
export const name = "Copy scene submap to background with tile offset (GBS 3)";
export const groups = ["EVENT_GROUP_SCREEN"];

export const autoLabel = (fetchArg) => {
  return `Copy scene submap to background with tile offset (GBS 3)`;
};
...
export const compile = (input, helpers) => {
  ...
  _addComment("Copy scene submap to background with tile offset (GBS 3)");
  ...
  _stackPushConst((input.dest_y << 8) | input.dest_x); 
  _stackPushConst((input.source_y << 8) | input.source_x); 
